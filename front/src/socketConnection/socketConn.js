...
// Pas de signalisation P2P (offer/answer/ice) : c'est le SFU LiveKit qui
// gère les flux média entre les clients et le serveur LiveKit.
...
// Reçoit l'URL + le jeton LiveKit propres à ce client (identité = socket.id).
export const onLiveKitInfo = (cb) => {
  socket.on('livekit-info', cb);
  return () => socket.off('livekit-info', cb);
};
