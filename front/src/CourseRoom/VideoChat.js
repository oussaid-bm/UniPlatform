...
const VideoChat = ({
  ...
  livekitUrl: initialLivekitUrl, livekitToken: initialLivekitToken,
}) => {
  ...
  const [livekitUrl, setLivekitUrl]    = useState(initialLivekitUrl || null);
  const [livekitToken, setLivekitToken] = useState(initialLivekitToken || null);
  ...
  // Connexion au SFU LiveKit dès qu'on a l'URL + le jeton et qu'on est en session
  useEffect(() => {
    if (!inVideoSession || !livekitUrl || !livekitToken || lkConnectedRef.current) return;
    ...
    const connectToLiveKit = async () => {
      try {
        ...
        // Les étudiants démarrent pistes coupées (réactivées quand on les interroge)
        if (!isProfessor) {
        ...
        await livekit.connect(livekitUrl, livekitToken, (feedId, display, mediaStream) => {
          setRemoteFeeds(prev => ({ ...prev, [feedId]: { stream: mediaStream, display } }));
        });

        await livekit.publish(stream);
        lkConnectedRef.current = true;
        console.log('[LiveKit] Connecté et publié sur la room.');
      } catch (err) {
        console.error('[LiveKit] Échec connexion/publication:', err);
        setMediaError('Erreur de connexion au serveur vidéo.');
      }
    };

    connectToLiveKit();
  }, [inVideoSession, livekitUrl, livekitToken, isProfessor, user?.username]);

  // Reçoit l'URL + le jeton LiveKit du serveur (notamment pour les étudiants
  // qui rejoignent après le démarrage de la session)
  useEffect(() => {
    const unsub = onLiveKitInfo(({ url, token }) => {
      setLivekitUrl(url);
  ...
  const startScreenShare = async () => {
    try {
      ...
      // Remplace la piste vidéo publiée sur LiveKit
      await livekit.replaceVideoTrack(screenTrack);
  ...
  const stopScreenShare = useCallback(() => {
    ...
    if (camTrack) {
      livekit.replaceVideoTrack(camTrack).catch(() => {});
    }
  ...
  useEffect(() => {
    ...
    if (isProfessor) {
      ...
      unsubGranted = onFloorGranted(() => {
        // Floor granted: enable tracks and reconfigure LiveKit publisher
        const s = localStreamRef.current;
        s?.getVideoTracks().forEach(t => { t.enabled = true; });
        s?.getAudioTracks().forEach(t => { t.enabled = true; });
        livekit.configureMedia({ audio: true, video: true });
      ...
      unsubRemoved = onFloorRemoved(() => {
        ...
        livekit.configureMedia({ audio: false, video: false });
  ...
  // Nettoyage UNIQUEMENT au démontage réel du composant.
  // (Ne PAS dépendre de `livekit` : le hook renvoie un nouvel objet à chaque
  //  render, ce qui ferait tourner ce cleanup à chaque re-render et couperait
  //  la connexion + la caméra.)
  useEffect(() => {
    return () => {
  ...
  const toggleMic = () => {
    ...
    t.enabled = !newMuted;
    livekit.configureMedia({ audio: !newMuted });
  ...
  const toggleCam = () => {
    ...
    if (t) {
      t.enabled = !t.enabled;
      livekit.configureMedia({ video: t.enabled });
      setCamOff(c => !c);
    }
  };
  ...
  // Build remote streams map (feedId → stream) from LiveKit hook
  const sfuRemoteStreams = livekit.remoteStreams;
  // Merge with our remoteFeeds state
  const allRemoteFeeds = { ...remoteFeeds };
  Object.entries(sfuRemoteStreams).forEach(([feedId, stream]) => {
